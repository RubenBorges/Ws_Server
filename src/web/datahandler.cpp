...
void handleDataSync(data::dataTransaction &tx) {
  switch (tx.cmd) {
    case data::OP::RX: { 
      ...
      tx.status = (res == data::FileResult::SUCCESS) ? data::Result::SUCCESS : data::Result::FAILURE;
      break;
    }
    ...
    case data::OP::DEL: {
      ...
      std::error_code ec; // Swapped to standard error code to preserve standalone compatibility boundaries
